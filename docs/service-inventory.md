...
|---|---|---|
| mDNS responder | Node.js scheduled task | Publishes `*.samurai.local` on the physical LAN |
| ingress-nginx | Docker Desktop Kubernetes | Routes HTTPS hostnames to application ports |
| Local CA | `%LOCALAPPDATA%\InstantPlex\pki` | Signs the wildcard local certificate |
| Tailscale | Windows service | Separate remote-access network |

## Other active Docker projects

| Project | Compose file | Exposed ports |
|---|---|---|
| [Immich](https://github.com/arbaaz360/immich) | `X:\Immich\docker-compose.yml` | 2283, 2298, 2299, 3111 |
...
- InstantPlex mDNS: scheduled task installed by `scripts\setup-mdns.ps1`.
